...
export class MarkdownEngine {
    ...
    public get engine(): MarkdownIt {
        if (!this.md) {
            ...
            let cfg = (key: string) => vscode.workspace.getConfiguration('markdown-handy').get(key)
            if (cfg('checkbox')) {
            ...
            if (cfg('emoji')) {
                ...
                this.md.renderer.rules.emoji = function(token, idx) {
                    ...
                    let emojidata = new Buffer(utils.readFile(emojipath, null)).toString('base64')
                    if (emojidata) {
            ...
            this.md.renderer.rules.image = function(tokens, idx, options, env, self) {
                ...
                if (['http', 'https'].indexOf(hrefUri.scheme) === -1) {
                    let data = new Buffer(utils.readFile(href, null)).toString('base64')
    ...
    public parse(document: vscode.Uri, source: string): Token[] {
